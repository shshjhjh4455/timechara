...
def main():
    ...
    for data_idx, example in tqdm(enumerate(dataset), total=len(dataset), ncols=120):
        ...
        if data_cnt % 10 == 0:
            ...
            with open(os.path.join(args.output_dir, 'test', f'{args.model_name}_{args.method_name}', f'eval_{args.eval_mode}', f'{args.output_fname}.temp'), 'w') as fp:
                json.dump(outputs, fp, indent=4)
            print(f"\n\nSaved outputs to {os.path.join(args.output_dir, 'test', f'{args.model_name}_{args.method_name}', f'eval_{args.eval_mode}', f'{args.output_fname}.temp')}!!\n\n")
