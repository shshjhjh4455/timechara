    ...
    for example in tqdm(dataset, ncols=120):
        ...
        if args.model_name in ['gpt-4o-2024-05-13', 'gpt-4-1106-preview', 'gpt-3.5-turbo-1106']:
            ...
            if args.method_name == 'zero-shot':
                ...
                response, hint, _ = narrative_experts(args.model_name, system_prompt, question, question_period, character, book_no, day, participants, series, tokenizer=tokenizer, model=model,
                                                   data_type=data_type, max_tokens=2048, temperature=0.2, top_p=1.0, n=1, seed=0, device=device)
            elif args.method_name == 'narrative-experts-rag-cutoff':
        ...
        if data_cnt % 10 == 0:
            ...
            with open(os.path.join(args.output_dir, 'test', f'{args.model_name}_{args.method_name}', f'{args.output_fname}.temp'), 'w') as fp:
                json.dump(outputs, fp, indent=4)
            print(f"\n\nSaved outputs to {os.path.join(args.output_dir, 'test', f'{args.model_name}_{args.method_name}', f'{args.output_fname}.temp')}!!\n\n")
